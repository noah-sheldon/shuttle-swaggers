...
import { getStartOfTodayUTC } from "@/lib/utils/dateHelpers";
...
export default function AdminPage() {
  ...
  const [configureSessionId, setConfigureSessionId] = useState<string | null>(
    null
  );
  ...
  const generateSessions = async () => {
    ...
    try {
      const startDate = getStartOfTodayUTC().toISOString();
  ...
  const saveSessionConfiguration = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/sessions/${configureSessionId}/update-config`,
        {
          method: "POST",
      ...
      if (response.ok) {
        ...
        toast.success(
          "Session configured successfully! Players have been added and settings saved."
        );
        ...
        toast.error(
          error.error || "Failed to configure session. Please try again."
        );
      }
    } catch (error) {
      console.error("Error configuring session:", error);
      toast.error(
        "Network error: Failed to configure session. Please check your connection and try again."
      );
    } finally {
  ...
  const addPlayerToSelection = (playerName: string) => {
    ...
    const nameExists = selectedPlayers.some(
  ...
  const addNewPlayer = () => {
    ...
    if (nameExists) {
      toast.error(
        "A player with this name already exists. Please use a different name."
      );
      return;
    }
  ...
  const selectAllPlayers = () => {
    const availableToSelect = availablePlayers.filter(
  ...
  const getSessionStatus = (session: Session) => {
    ...
    if (session.player_data && session.player_data.length > 0)
      return "configured";
    return "pending";
  };
    ...
    try {
      const response = await fetch(`/api/sessions/${liveSession._id}/control`, {
        ...
        body: JSON.stringify({
          action:
            type === "session"
              ? "reset_session_ratings"
              : "reset_historical_ratings",
        }),
      });

      if (response.ok) {
        toast.success(
          `${type === "session" ? "Session ratings" : "Historical data"} reset successfully`
        );
    ...
    <div className="pt-16 min-h-screen bg-gray-50">
      <div className="container mx-auto px-6 py-12">
        <div className="max-w-6xl mx-auto">
              ...
              <CardContent className="p-4">
                {result.error ? (
                  ...
                  <div className="text-green-600">
                    ...
                    <div>
                      Created: {result.sessions?.length || 0} new sessions
                    </div>
            ...
            {loadingSessions ? (
              ...
              <Card className="p-8 text-center">
                ...
                <p className="text-gray-500 mb-6">
                  Create your first session or generate bulk sessions to get
                  started.
                </p>
              ...
              <div className="grid lg:grid-cols-2 xl:grid-cols-3 gap-6">
                {sessions.map((session) => {
                  ...
                  const courtCount =
                    session.config?.court_count || session.courts || 0;
                      ...
                      <div className="p-6">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <h3 className="font-semibold text-[#004d40] text-lg">
                                {session.location === "Watford Central"
                                  ? "Watford Central"
                                  : "Fuller Health"}
                              </h3>
                            </div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {sessionStatus === "live" && (
                            <Button
                              onClick={() =>
                                manageLiveSession(session._id!, "end")
                              }
          ...
          <Dialog
            open={!!configureSessionId}
            onOpenChange={() => setConfigureSessionId(null)}
          >
            <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle className="text-2xl text-[#004d40]">
                  Configure Session
                </DialogTitle>
              </DialogHeader>
                ...
                <TabsContent value="settings" className="space-y-4">
                  <div className="space-y-6">
                    <div className="space-y-2">
                      <Label
                        htmlFor="courtCount"
                        className="text-lg font-semibold text-[#004d40]"
                      >
                        Number of Courts
                      </Label>
                      <Select
                    ...
                    <div className="space-y-3 p-4 bg-blue-50 rounded-lg">
                      <h4 className="font-semibold text-[#004d40]">
                        Session Details
                      </h4>
